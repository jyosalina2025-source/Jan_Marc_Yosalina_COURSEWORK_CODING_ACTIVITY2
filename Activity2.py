...
def save_contacts(filename, contacts):
    with open(filename, mode='w', newline='') as file:
        writer = csv.DictWriter(file, fieldnames=["Name", "Phone", "Email", "Address", "Socials"])
        writer.writeheader()
        writer.writerows(contacts)

# Function to display contacts
def display_contacts(contacts):
    print("\n{:<20} {:<15} {:<30} {:<30} {:<30}".format("Name", "Phone", "Email","Address","Socials"))
    print("-" * 125)
    for contact in contacts:
        print("{:<20} {:<15} {:<30} {:<30} {:<30}".format(contact['Name'], contact['Phone'], contact['Email'], contact['Address'], contact['Socials']))
...
def add_contact(contacts):
    ...
    socials = input("Enter Social Media:")
    contacts.append({"Name": name, "Phone": phone, "Email": email, "Address": address, "Socials": socials})
...
def update_contact(contacts):
    ...
    for contact in contacts:
        if contact['Name'].lower() == name.lower():
            ...
            contact['Socials'] = input(f"Enter new Social Media for {name}: ")
            found = True
